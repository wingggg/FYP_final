...
static inline l_list *l_nextl(l_list *link) { return link->forward;  }
static inline l_list *l_prevl(l_list *link) { return link->backward; }
static inline int    l_lempty(l_list *link) { return (link->forward == link); }
#endif
...
static inline void l_linit(l_list *link) { link->forward = link->backward = link; }
